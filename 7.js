...
class Participant {
  ...
  // Він приймаєprepareMessage один параметр - text - текст повідомлення, яке потрібно відправити.
  // Метод  приймає text та повертає  `[${this.alias}]: ${text}`
}

// Клас SMSCommunicator відповідає за відправку повідомлень через SMS.
class SMSCommunicator {
  static transmit(message) {
    return console.log(`Відправлено SMS: ${message}`);
  }
...
class EmailCommunicator {
  static transmit(message) {
    return console.log(`Відправлено Email: ${message}`);
  }
